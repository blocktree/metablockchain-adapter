...
//MMUIBlockScanner metablockchain的区块链扫描器
type MMUIBlockScanner struct {
...
//NewMMUIBlockScanner 创建区块链扫描器
func NewMMUIBlockScanner(wm *WalletManager) *MMUIBlockScanner {
	bs := MMUIBlockScanner{
...
func (bs *MMUIBlockScanner) SetRescanBlockHeight(height uint64) error {
	height = height - 1
	if height < 0 {
...
//ScanBlockTask 扫描任务
func (bs *MMUIBlockScanner) ScanBlockTask() {
...
//ScanBlock 扫描指定高度区块
func (bs *MMUIBlockScanner) ScanBlock(height uint64) error {
...
func (bs *MMUIBlockScanner) scanBlock(height uint64) (*Block, error) {
...
func (bs *MMUIBlockScanner) ScanTxMemPool() {
...
func (bs *MMUIBlockScanner) RescanFailedRecord() {
...
func (bs *MMUIBlockScanner) newBlockNotify(block *Block, isFork bool) {
...
func (bs *MMUIBlockScanner) BatchExtractTransaction(blockHeight uint64, blockHash string, txs []Transaction, memPool bool) error {
...
func (bs *MMUIBlockScanner) extractRuntime(producer chan ExtractResult, worker chan ExtractResult, quit chan struct{}) {
...
func (bs *MMUIBlockScanner) ExtractTransaction(blockHeight uint64, blockHash string, transaction Transaction, scanAddressFunc openwallet.BlockScanTargetFunc) ExtractResult {
...
func (bs *MMUIBlockScanner) extractTransaction(trx *Transaction, result *ExtractResult, scanTargetFunc openwallet.BlockScanTargetFunc) {
	...
	//整数转小数
	for index, trxDetail := range trx.ToTrxDetailArr{
		amountUint, _ := strconv.ParseUint(trxDetail.Amount, 10, 64)
		amount_dec, _ := decimal.NewFromString(convertToAmount( amountUint, bs.wm.GetDecimal()))
	...
	//订阅地址为交易单中的发送者
	accountID1, ok1 := scanTargetFunc(openwallet.ScanTarget{Address: from, Symbol: bs.wm.Symbol(), BalanceModelType: openwallet.BalanceModelTypeAccount})
	//订阅地址为交易单中的接收者
	if ok1 {
...
func (bs *MMUIBlockScanner) InitExtractResult(sourceKey string, tx *Transaction, result *ExtractResult) {
	...
	amount_dec, _ := decimal.NewFromString(convertToAmount(tx.Amount, bs.wm.GetDecimal()))
	...
	transx := &openwallet.Transaction{
		Fees:        convertToAmount(tx.Fee, bs.wm.GetDecimal()),
		Coin:        coin,
	...
	transx.SetExtParam("memo", tx.FromTrxDetailArr[0].Memo)
...
func (bs *MMUIBlockScanner) InitExtractOutputResult(tx *Transaction, result *ExtractResult, scanTargetFunc openwallet.BlockScanTargetFunc) {
	status := "1"
	reason := ""
	...
	transx.SetExtParam("memo", tx.ToTrxDetailArr[0].Memo)
...
func (bs *MMUIBlockScanner) extractTxInput(trx *Transaction, txExtractData *openwallet.TxExtractData) {
...
func (bs *MMUIBlockScanner) newExtractDataNotify(height uint64, extractData map[string]*openwallet.TxExtractData) error {
...
func (bs *MMUIBlockScanner) DeleteUnscanRecordNotFindTX() error {
...
//func (bs *MMUIBlockScanner) SaveRechargeToWalletDB(height uint64, list []*openwallet.Recharge) error {
//
//	for _, r := range list {
//
...
func (bs *MMUIBlockScanner) GetCurrentBlockHeader() (*openwallet.BlockHeader, error) {
	var (
...
func (bs *MMUIBlockScanner) GetScannedBlockHeader() (*openwallet.BlockHeader, error) {
...
func (bs *MMUIBlockScanner) GetScannedBlockHeight() uint64 {
	localHeight, _, _ := bs.wm.Blockscanner.GetLocalNewBlock()
	return localHeight
}

//func (bs *MMUIBlockScanner) ExtractTransactionData(txid string, scanTargetFunc openwallet.BlockScanTargetFunc) (map[string][]*openwallet.TxExtractData, error) {
//
...
//func (bs *MMUIBlockScanner) DropRechargeRecords(accountID string) error {
//	bs.mu.RLock()
//	defer bs.mu.RUnlock()
//
...
//func (bs *MMUIBlockScanner) DeleteRechargesByHeight(height uint64) error {
//
//	bs.mu.RLock()
//	defer bs.mu.RUnlock()
//
...
func (bs *MMUIBlockScanner) GetSourceKeyByAddress(address string) (string, bool) {
	bs.Mu.RLock()
	defer bs.Mu.RUnlock()
...
// func (bs *MMUIBlockScanner) GetWalletByAddress(address string) (*openwallet.Wallet, bool) {
// 	bs.mu.RLock()
// 	defer bs.mu.RUnlock()
...
func (bs *MMUIBlockScanner) GetLocalNewBlock() (uint64, string, error) {
...
func (bs *MMUIBlockScanner) SaveLocalNewBlock(blockHeight uint64, blockHash string) error {
...
func (bs *MMUIBlockScanner) GetBalanceByAddress(address ...string) ([]*openwallet.Balance, error) {
	...
	for _, addr := range address {
		...
		addrsBalance = append(addrsBalance, &openwallet.Balance{
			Symbol:  bs.wm.Symbol(),
			Address: addr,
			Balance: convertToAmount(uint64(balance.Balance.Int64()), bs.wm.GetDecimal()),
		})
	}
...
//Run 运行
func (bs *MMUIBlockScanner) Run() error {
...
////Stop 停止扫描
func (bs *MMUIBlockScanner) Stop() error {
...
//Pause 暂停扫描
func (bs *MMUIBlockScanner) Pause() error {
...
//Restart 继续扫描
func (bs *MMUIBlockScanner) Restart() error {
...
func (bs *MMUIBlockScanner) setupSocketIO() error {
	return nil
}

//SupportBlockchainDAI 支持外部设置区块链数据访问接口
//@optional
func (bs *MMUIBlockScanner) SupportBlockchainDAI() bool {
	return true
}
